/*
无旋treap: tree + heap + split merge
相同键值的节点不去重 同时存在

支持区间操作:利用 split_by_size 分裂两次 将区间[l,r]分裂出来 然后操作

区间翻转: 需要翻转时交换左右儿子 然后递归到左右儿子 使用懒标记保证复杂度
          注意split和merge在递归之前要先pushdown
区间插入: 在pos后面插入一个序列 split_by_size 按pos分裂成x,z  O(n)将序列建成树y 合并x,y,z
区间删除: 将区间分离出来直接删除

(1)  get_node()
(2)  pushup()   split merge 递归后需要pushup
(3)  pushdown() 区间操作有懒标记时使用   split merge 递归前需要pushdown
(4)  split_by_key() 按值分裂 
(5)  split_by_size() 按大小分裂 区间操作时值不再满足二叉树性质
(6)  merge() 合并时维护堆的性质
(7)  insert()
(8)  delete()
(9)  get_rank()
...
(7)  merge()是保证二叉树高度的关键
(9)(10)(11)(12) 可以用split merge 拼凑 也可以用普通二叉树的方法
